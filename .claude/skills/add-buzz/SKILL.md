...
An agent caller's target identity is derived from its own wiring. Passing `--instance` as an agent is only valid when it names that agent's own resolved identity (redundant but harmless) — passing any other value is a **hard error**, not a silent redirect to the agent's own identity. (An earlier version got this wrong: it silently discarded a mismatched `--instance` from an agent and fell back to the caller's own identity with no error — live-tested consequence was three in-agent calls each meant for a different persona instead silently overwriting the calling agent's own profile three times in a row. Fixed — see `resolveInstance` in `cli-buzz.ts`.) Only callers whose agent group has a Buzz channel wired can reach the command at all — an agent group with no Buzz relationship gets a clear error, not a silent no-op; an agent group wired to more than one Buzz instance also errors (ambiguous, not expected under the current one-instance-per-agent-group design). It fetches the current profile and merges rather than overwriting (kind:0 is a Nostr *replaceable* event — a naive publish of only the passed fields would erase everything else already set), and verifies the publish actually landed on the relay before reporting success.
