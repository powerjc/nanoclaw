...
async function main(): Promise<void> {
  // Start the in-process translator proxy (Anthropic API -> Gemini API)
  const proxyPort = await startProxy();

  // Override the environment variables for the Claude SDK
  ...
  try {
    while (true) {
      ...
      // -- INJECT MODEL SWITCHER INTERCEPTOR --
      if (typeof prompt === 'string' && prompt.startsWith('/model ')) {
        const targetModel = prompt.split(' ')[1]?.trim();
        if (targetModel) {
          log(`Switching model to ${targetModel}`);
          fs.writeFileSync('/workspace/group/.current-model', targetModel);
          writeOutput({ status: 'success', result: `*Model switched to ${targetModel}*`, newSessionId: sessionId });
        } else {
          writeOutput({ status: 'error', result: 'Usage: /model <provider-name> (e.g. /model gemini)', newSessionId: sessionId });
        }

        // Wait for next message, skipping the agent API call for this turn
        const skippedMessage = await waitForIpcMessage();
        if (skippedMessage === null) break;
        prompt = skippedMessage;
      }
      // -- END INJECTION --
