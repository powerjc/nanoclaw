...
export async function processTaskIpc(
  data: {
    ...
    chatJid?: string;
    targetJid?: string;
    // For register_group and update_group
    jid?: string;
    name?: string;
    folder?: string;
    trigger?: string;
  ...
  switch (data.type) {
    ...
    case 'update_group':
      ...
      if (!registeredGroups[data.jid]) {
        logger.warn(
          { jid: data.jid },
          'update_group: target group not registered',
        );
        break;
      }
      {
        ...
        if (data.requiresTrigger !== undefined)
          patch.requiresTrigger = data.requiresTrigger;
        if (data.containerConfig !== undefined)
          patch.containerConfig = data.containerConfig;
        deps.updateGroup(data.jid, patch);
        logger.info(
          { jid: data.jid, fields: Object.keys(patch) },
