...
export function patchRegisteredGroup(
  jid: string,
  patch: { requiresTrigger?: boolean; containerConfig?: ContainerConfig },
): boolean {
  const setParts: string[] = [];
  const values: unknown[] = [];

  if (patch.requiresTrigger !== undefined) {
    setParts.push('requires_trigger = ?');
    values.push(patch.requiresTrigger ? 1 : 0);
  }
  if (patch.containerConfig !== undefined) {
    setParts.push('container_config = ?');
    values.push(JSON.stringify(patch.containerConfig));
  }
